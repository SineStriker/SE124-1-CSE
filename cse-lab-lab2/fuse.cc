...
//
void fuseserver_setattr(fuse_req_t req, fuse_ino_t ino, struct stat *attr, int to_set,
...
void fuseserver_read(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off,
    ...
    if (chfs->read(inum, size, off, str) != chfs_client::OK) {
        fuse_reply_err(req, ENOSYS);
        return;
    }
    ...
    memcpy(buf, str.c_str(), realSize);

    fuse_reply_buf(req, buf, realSize);

    std::cout << "read file size" << realSize << std::endl;
...
//
void fuseserver_write(fuse_req_t req, fuse_ino_t ino, const char *buf, size_t size, off_t off,
                      struct fuse_file_info *fi) {
#if 1
    // Change the above line to "#if 1", and your code goes here
    ...
    size_t size2;
    if (chfs->write(inum, size, off, buf, size2) != chfs_client::OK) {
        fuse_reply_err(req, ENOSYS);
        return;
    }
    fuse_reply_write(req, size2);
...
//
chfs_client::status fuseserver_createhelper(fuse_ino_t parent, const char *name, mode_t mode,
                                            struct fuse_entry_param *e, int type) {
    int ret;
...
void fuseserver_create(fuse_req_t req, fuse_ino_t parent, const char *name, mode_t mode,
                       struct fuse_file_info *fi) {
    struct fuse_entry_param e;
    chfs_client::status ret;
    if ((ret = fuseserver_createhelper(parent, name, mode, &e, extent_protocol::T_FILE)) ==
        chfs_client::OK) {
...
void fuseserver_mknod(fuse_req_t req, fuse_ino_t parent, const char *name, mode_t mode,
                      dev_t rdev) {
    struct fuse_entry_param e;
    chfs_client::status ret;
    if ((ret = fuseserver_createhelper(parent, name, mode, &e, extent_protocol::T_FILE)) ==
...
void fuseserver_readlink(fuse_req_t req, fuse_ino_t ino) {
    ...
    if (chfs->readlink(ino, str) != chfs_client::OK) {
        fuse_reply_err(req, ENOENT);
        return;
    }
...
void fuseserver_open(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi) {
...
void fuseserver_mkdir(fuse_req_t req, fuse_ino_t parent, const char *name, mode_t mode) {
    ...
    chfs_client::inum iparent = parent, inum;

    if (chfs->mkdir(iparent, name, mode, inum) != chfs_client::OK) {
        fuse_reply_err(req, ENOSYS);
        return;
    }
